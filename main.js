...
let canvasWProp          = null;
let canvasHProp          = null;
...
function detectDegradedGPU() {
  if (_isIOS || _isAndroid) return true; // benchmark decides for all mobile
  try {
    const gl = document.createElement("canvas").getContext("webgl2")
            || document.createElement("canvas").getContext("webgl");
    if (gl && gl.getExtension("WEBGL_debug_renderer_info")) return false;
  } catch (_) {}
  return true;
}
